function initTask(subTask) {
   ...
   subTask.loadLevel = function(curLevel) {
      ...
      permutation = [];
      for(var iPos = 0; iPos < nDigit; iPos++){
         permutation[iPos] = iPos;
      }
      rng.shuffle(permutation);
      for(var iPos = 0; iPos < nDigit; iPos++){
         vigenereKey[iPos] = rng.nextInt(1,25) ;
      }
   ...
   function encrypt(plain) {
       ...
       if (level=="easy"){
          for(var iLetter = 0; iLetter < plain.length; iLetter++){
             var letter = plain[iLetter];
             var letterIndex = Beav.Array.indexOf(letters,letter);
             var vigenereLetter = letters[(letterIndex + vigenereKey[iLetter % vigenereKey.length]) % letters.length];
             var newIndex = permutation[iLetter];
             encrypted[newIndex] = vigenereLetter;
          }
      }
      if (level=="medium"){
         var permutedPlain = []
         for(var iLetter = 0; iLetter < plain.length; iLetter++){
            ...
            var letterIndex = Beav.Array.indexOf(letters,letter);
            var newIndex = permutation[iLetter];
            permutedPlain[newIndex] = letter;               
         }
         for(var iLetter = 0; iLetter < plain.length; iLetter++){
            var thisLetter = permutedPlain[iLetter];
            if (iLetter==0){
               var prevLetter = letters[0];
            }           
            else {
               var prevLetter = encrypted[iLetter-1];
            }
            var thisLetterIndex = Beav.Array.indexOf(letters,thisLetter);
            var prevLetterIndex = Beav.Array.indexOf(letters,prevLetter);
            var newLetter = letters[(thisLetterIndex + prevLetterIndex) % letters.length];
                encrypted[iLetter] = newLetter;               
         }
      }
      if (level=="hard"){
         var encryptedBeforePermutation = []
         for(var iLetter = 0; iLetter < plain.length; iLetter++){
            var thisLetter = plain[iLetter];
            if (iLetter==0){
               var prevLetter = letters[0];
            }           
            else {
               var prevLetter = encryptedBeforePermutation[iLetter-1];
            }
            var thisLetterIndex = Beav.Array.indexOf(letters,thisLetter);
            var prevLetterIndex = Beav.Array.indexOf(letters,prevLetter);
            var newLetter = letters[(thisLetterIndex + prevLetterIndex) % letters.length];
               encryptedBeforePermutation[iLetter] = newLetter;               
         }
         for(var iLetter = 0; iLetter < plain.length; iLetter++){
            var letter = encryptedBeforePermutation[iLetter];
            var letterIndex = Beav.Array.indexOf(letters,letter);
            var newIndex = permutation[iLetter];
