function initTask(subTask) {
   ...
   function angleToWheelCenter(iWheel, x, y) {
      ...
      var xCenter = $("#vault").offset().left + wheelCenter[iWheel][0]*scale;
      if(Beav.Navigator.isIE8()){
         var yCenter = $("#vault").offset().top + document.documentElement.scrollTop + wheelCenter[iWheel][1]*scale;
      }else{
         var yCenter = $("#vault").offset().top + wheelCenter[iWheel][1]*scale;
      }
      var angle = (Math.atan2(y - yCenter, x - xCenter) + 5 * Math.PI / 2) % (Math.PI * 2);
